...
-- //////////////////////////////////////////////////////////////
--		CONTENIDO DEL SP
--	[PG_VALIDACION_LOCACION_ORIGEN]
--	[PG_VALIDACION_LOCACION_DESTINO_X_FOLIO]
--	[PG_VALIDACION_LOCACION_DESTINO_X_FOLIO_TIPO_ITEM]
--	[PG_VALIDACION_SALIDA_MQU]
--	[PG_VALIDACION_EXISTE_ORDEN]
--***********	STORED PROCEDURE ---> PROCESO PARA EL MOVIMIENTO ENTRE LOCACIONES
--	[PG_PR_MOVIMIENTO_MHIB_A_MHIA]
--	[PG_PR_MOVIMIENTO_LOCACION_A_LOCACION]
--	[PG_PR_MOVIMIENTO_ENTRE_LOCACION_X_REGISTROS]
--	[PG_PR_MOVIMIENTO_ENTRE_LOCACION_X_FOLIO]
...
DECLARE @VP_MENSAJE						VARCHAR(500) = ''
	...
	IF @PP_K_LC_DESTINO NOT IN(	SELECT	A4GLIDENTITY
								FROM	IMLOCFIL_SQL	(NOLOCK)
								WHERE	LOC  IN (	'MHI'	,'MIT'	,'MQU'	,'MRD'	,'MCT'	,'MFP' ,'PRD'	)
								UNION
								SELECT	A4GLIDENTITY
								FROM	IMLOCFIL_SQL	(NOLOCK)
								WHERE	LOC NOT LIKE 'T%'	AND LOC NOT LIKE 'G%'	)
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_VALIDACION_LOCACION_DESTINO_X_FOLIO_TIPO_ITEM]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_VALIDACION_LOCACION_DESTINO_X_FOLIO_TIPO_ITEM]
GO
--		 EXECUTE [dbo].[PG_VALIDACION_LOCACION_DESTINO_X_FOLIO_TIPO_ITEM]
CREATE PROCEDURE [dbo].[PG_VALIDACION_LOCACION_DESTINO_X_FOLIO_TIPO_ITEM]
	@PP_K_SISTEMA_EXE				INT,
	@PP_K_USUARIO_ACCION			INT,
	-- ===========================
	@PP_K_ITEM						INT,
...
AS
DECLARE  @VP_MENSAJE			VARCHAR(500) = ''
		,@VP_ES_MULLER			VARCHAR(50)

	--	SE AGREGA VALIDACIÓN ESPECIAL, SI EL ITEM NO ES DE TIPO MULLER NO SE PODRÁ MOVER A P07	
	SELECT	@VP_ES_MULLER		= TRADEMARK_ITEM
	FROM	COMPRAS.dbo.ITEM	(NOLOCK)
	WHERE	K_ITEM = @PP_K_ITEM
	
	IF	@VP_ES_MULLER = 'MULLER'
	BEGIN
		IF @PP_K_LC_DESTINO NOT IN(	SELECT	A4GLIDENTITY
									FROM	IMLOCFIL_SQL	(NOLOCK)
									WHERE	LOC  IN (	'MHI' , 'MQU' ,'P07')	)
		BEGIN
			SET @VP_MENSAJE='Proporcione una Locación destino válida para la transferencia....[MULLER]'
			RAISERROR (@VP_MENSAJE, 16, 1 )
		END
	END
	ELSE IF	@VP_ES_MULLER = 'HILO'
	BEGIN
		IF @PP_K_LC_DESTINO NOT IN(	SELECT	A4GLIDENTITY
									FROM	IMLOCFIL_SQL	(NOLOCK)
									WHERE	LOC  IN (	'MHI' , 'MQU' ,'PRD')	)
		BEGIN
			SET @VP_MENSAJE='Proporcione una Locación destino válida para la transferencia....[HILO]'
			RAISERROR (@VP_MENSAJE, 16, 1 )
		END
	END
	ELSE
...
BEGIN TRY
	...
	INNER	JOIN DATA_02.DBO.IMLOCFIL_SQL	(NOLOCK) ON CCJOBHDR_SQL.machine = IMLOCFIL_SQL.loc_desc
...
GO
--		 EXECUTE [dbo].[PG_PR_MOVIMIENTO_LOCACION_A_LOCACION]	0,139, 3005,1,70,40003,4,125,3068,	1,1,122.0000,2619,-1
CREATE PROCEDURE [dbo].[PG_PR_MOVIMIENTO_LOCACION_A_LOCACION]
	...
	@PP_CANTIDAD_MOVIMIENTO			[DECIMAL](19,4),		--	ESTA CANTIDAD ES PARA ENVIAR PARCIALES A OTRAS LOCACIONES.
	@PP_K_FOLIO_DESTINO				[INT],
	@PP_K_ORDEN_TRABAJO_DESTINO		[INT], --	--	AX: 20210521 SE DEBE COLOCAR EL VALOR PREDETERMINADO -1, DE DONDE NO SE ENVÍE EL PARAMETRO.
	@PP_K_FOLIO_ORIGEN				[INT],		--	AX: 20210521
	-- ===========================
	@PP_K_ORDEN_TRABAJO_ORIGEN		[INT],
	@PP_TIPO_FOLIO_ORIGEN			[VARCHAR] (3)
AS
DECLARE @VP_MENSAJE						VARCHAR(500) = ''		--,@VP_LOCACION_ORIGEN_BD		INT
		,@VP_QTY_MOVIMIENTO				DECIMAL(19,4)			--,@VP_K_FOLIO_ORIGEN			INT
		--,@VP_K_ORDEN_TRABAJO_ORIGEN		INT						,@VP_TIPO_FOLIO_ORIGEN		VARCHAR(3)
...
--=====================================================================================================================================
	...
	--SELECT	--	@VP_K_FOLIO_ORIGEN			= INVENTARIO.K_FOLIO,				--	AX: 20210521
	--		@VP_LOCACION_ORIGEN_BD		= FOLIO.K_LOCACION,
	--		@VP_K_ORDEN_TRABAJO_ORIGEN	= K_ORDEN_TRABAJO,
	--		@VP_TIPO_FOLIO_ORIGEN		= TIPO	
	--FROM	INVENTARIO		(NOLOCK) 
	...
	-- SE VERIFICA QUE EL FOLIO ORIGEN CUENTE CON UN FOLIO ASIGNADO.
	--IF @VP_K_FOLIO_ORIGEN IS NULL OR @VP_K_FOLIO_ORIGEN=0
	IF ( @PP_K_FOLIO_ORIGEN IS NULL ) OR ( @PP_K_FOLIO_ORIGEN = 0 )			--	AX: 20210521
	BEGIN
		SET @VP_MENSAJE='Folio Origen [0] no encontrado...Verifique'
		RAISERROR (@VP_MENSAJE, 16, 1 )
	END
	...
	---- SE VERIFICA NUEVAMENTE LA LOCACIÓN ORIGEN YA QUE EL SISTEMA NO CUENTA CON UN PROCESO DE BLOQUEO DE REGISTROS, Y SE PUEDE DAR EL CASO
	---- QUE DOS USUARIOS REALICEN EL MOVIMIENTO AL MISMO TIEMPO.
...
--=====================================================================================================================================
	...
	EXECUTE [dbo].[PG_IN_MOVIMIENTO_INVENTARIO]	@PP_K_SISTEMA_EXE, @PP_K_USUARIO_ACCION
											,@PP_K_ORDEN_COMPRA_PEDIDO		,@PP_K_ENTREGA
											,@PP_K_ITEM						,@PP_LOTE_PEARL
											,@PP_K_LOCACION_ORIGEN			,@PP_K_FOLIO_ORIGEN		---	@VP_K_FOLIO_ORIGEN				--- AX:20210521
...
--	TIPO_ACCION	#1 FOLIO_NUEVO	, #2 FOLIO_EXISTENTE	
	...
	BEGIN
		...
		WHERE	K_FOLIO		= @PP_K_LC_DES		-- CUANDO ES ESTE TIPO DE ACCIÓN SE RECIBE DEL FRONT EL FOLIO Y NO LA LOCACIÓN.
			...
			EXECUTE	[dbo].[PG_VALIDACION_LOCACION_DESTINO_X_FOLIO_TIPO_ITEM]	@PP_K_SISTEMA_EXE,	@PP_K_USUARIO_ACCION,
																				-- ===========================
																				@PP_K_ITEM		,	@VP_LOCACION_DESTINO_DE_FOLIO
		...
		DECLARE		@VP_K_ORDEN_TRABAJO		INT			,@VP_TIPO			VARCHAR(3)
						
			SELECT	@VP_TIPO			= ISNULL(TIPO				,	'B'),
					@VP_K_ORDEN_TRABAJO	= ISNULL(K_ORDEN_TRABAJO	,	0)
			FROM	FOLIO	(NOLOCK)
			WHERE	K_FOLIO = @PP_K_FO_ORI
			IF @@ROWCOUNT = 0
			BEGIN
				RAISERROR ('No se pudo encontrar la información del FOLIO origen...Verifique!', 16, 1 )
			END		
	END
...
--	LIVE
--		 EXECUTE [dbo].[PG_PR_MOVIMIENTO_ENTRE_LOCACION_X_FOLIO] 1,139,  1 , 70 , 4 , 125 , 2619
--		 EXECUTE [dbo].[PG_PR_MOVIMIENTO_ENTRE_LOCACION_X_FOLIO] 1,87,  1 , 459 , 4 , 129 , 2659 
...
DECLARE  @VP_CU_K_ORDE				VARCHAR(500)		,@VP_CU_K_ENTR				VARCHAR(500)		,@VP_CU_LOTE_P				VARCHAR(500)		
		...
		-- ======================================================
		,@VP_K_ORDEN_TRABAJO		INT					,@VP_TIPO					VARCHAR (3)
...
DECLARE CU_OBTENER_REGISTROS			CURSOR LOCAL FOR
	...
	OPEN	CU_OBTENER_REGISTROS
		...
		BEGIN	
			...
			--													,@VP_CU_QTY_MV		,@VP_K_FOLIO_DESTINO
			--													,@VP_ORDEN_DESTINO	,@PP_K_FOLIO_ORIGEN
			--END
		--==============================================================
		--DESPUES DEL MOVIMIENTO SE VERIFICA LA CANTIDAD EN INVENTARIO, SI ESTÁ POR DEBAJO DEL MÍNIMO Y NO SE HA ENVIADO NOTIFICACIÓN SE ENVÍA AL MOMENTO.
		DECLARE		@PP_PART_NUMBER_ITEM_PEARL				VARCHAR(250),
					@PP_D_ITEM								VARCHAR(250),
					-----================================================
					@PP_TIPO_ITEM							VARCHAR(50)


		SELECT	@PP_PART_NUMBER_ITEM_PEARL	= PART_NUMBER_ITEM_PEARL,
				@PP_D_ITEM					= D_ITEM,
				@PP_TIPO_ITEM				= TRADEMARK_ITEM
		FROM	COMPRAS.DBO.ITEM		(NOLOCK)
		WHERE	K_ITEM	= @PP_K_ITEM	--1504

			IF @PP_TIPO_ITEM = 'MULLER'
			BEGIN
				
				EXECUTE [dbo].[PG_NOTIFICAR_MULLER]		1,
						-----=============================
						@PP_K_ITEM,
...
END CATCH	
	...
	BEGIN
		SET		@VP_MENSAJE = 'No es posible [Mover] el registro: ' + @VP_MENSAJE
	END
	SELECT	@VP_MENSAJE AS MENSAJE, @VP_K_FOLIO_DESTINO	 AS CLAVE, @PP_TIPO_ACCION_FRONT AS TIPO_ACCION, 0 AS ORDEN_DESTINO		--@VP_ORDEN_DESTINO AS ORDEN_DESTINO -- AX:20210525
	-- //////////////////////////////////////////////////////////////
GO