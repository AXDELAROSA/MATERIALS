...
AS
	...
	AND			INVENTARIO.K_STATUS_INVENTARIO=10	-- [20]	= INSPECCIONADO
...
AS			
	...
	BEGIN
		--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
		SET @VP_MENSAJE='El folio ya se encuentra asignado, y no puede duplicarse... Verifique'
		RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
	END		
	...
	--============================================================================
		...
		VALUES	
			...
			IF @@ROWCOUNT = 0
				BEGIN
					--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
					SET @VP_MENSAJE='El movimiento x registro no fue insertado...Verifique'
					RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
				END				
	END
...
-- //	SE REALIZA DESDE QUE EL MATERIAL INGRESA A INVENTARIO
-- //////////////////////////////////////////////////////////////
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_IN_ARRAY_MOVIMIENTO_X_REGISTRO]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_IN_ARRAY_MOVIMIENTO_X_REGISTRO]
GO

CREATE PROCEDURE [dbo].[PG_IN_ARRAY_MOVIMIENTO_X_REGISTRO]
	@PP_K_SISTEMA_EXE				INT,
	@PP_K_USUARIO_ACCION			INT,
	-- ===========================
...
AS			
	DECLARE @VP_MENSAJE						VARCHAR(500) = ''
-- /////////////////////////////////////////////////////////////////////
	DECLARE	@VP_K_INVENTARIO						INT
			,@VP_K_FOLIO_ORIGEN						INT
			,@VP_K_ORDEN_TRABAJO_ORIGEN				INT
			,@VP_K_FOLIO_DESTINO					INT
			,@VP_K_ORDEN_TRABAJO_DESTINO			INT
			,@VP_CANTIDAD_MOVIMIENTO				DECIMAL(19,4)
	
	
	DECLARE CU_OBTENER_REGISTROS			CURSOR LOCAL FOR
		SELECT	K_INVENTARIO	,INVENTARIO.K_FOLIO		,FOLIO.K_ORDEN_TRABAJO		,CANTIDAD_RECIBIDA		,SERIE_NO
		FROM	INVENTARIO
		INNER JOIN FOLIO	ON	INVENTARIO.K_FOLIO=FOLIO.K_FOLIO
		WHERE	K_ITEM=82--@PP_K_ITEM 
		AND		LOTE_PEARL=201000001--@PP_LOTE_PEARL
		AND		K_ORDEN_COMPRA_PEDIDO	='00296-00001'--@PP_K_ORDEN_COMPRA_PEDIDO
		AND		K_ENTREGA				=1--@PP_K_ENTREGA
	...
	OPEN	CU_OBTENER_REGISTROS
		FETCH NEXT FROM CU_OBTENER_REGISTROS INTO @PP_CU_K_QUOTE_TRIM_COLOR
...
CREATE PROCEDURE [dbo].[PG_UP_INVENTARIO_LOCACION]
	...
	-- ===========================
	@PP_QTY_MOVIMIENTO				[DECIMAL](19,4),
	@PP_SUMA_O_RESTA				[VARCHAR](50)='SUMAR'
AS
DECLARE @VP_MENSAJE					VARCHAR(500) = ''
--============================================================================
...
--============================================================================
	...
	BEGIN
			...
			SET		CANTIDAD_DISPONIBLE=(CANTIDAD_DISPONIBLE +		@PP_QTY_MOVIMIENTO	)
			WHERE	K_ITEM		=@PP_K_ITEM	
...
-- //////////////////////////////////////////////////////////////
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_IN_TRX_INVENTARIO]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_IN_TRX_INVENTARIO]
GO

CREATE PROCEDURE [dbo].[PG_IN_TRX_INVENTARIO]
	@PP_K_SISTEMA_EXE				INT,
	@PP_K_USUARIO_ACCION			INT,
	-- ===========================
	@PP_K_ORDEN_COMPRA_PEDIDO			VARCHAR(50),
	...
	BEGIN
		...
		VALUES	
			(	@PP_K_LOCACION_ORIGEN						--	DEBE SER # 107=	RW0
				...
				,10											--	@PP_K_INVENTARIO_MOVIMIENTO_TIPO	= 'ENTRADA X LIBERACION'
				-- =========================	
				,@PP_K_ITEM
				,@PP_LOTE_PEARL				
				-- =========================
...
--=====================================================================================================================================
	IF @VP_MENSAJE=''
	BEGIN
		EXECUTE [dbo].[PG_IN_TRX_INVENTARIO]	@PP_K_SISTEMA_EXE, @PP_K_USUARIO_ACCION
...
--=====================================================================================================================================	
		...
		BEGIN			
			...
			END
				...
				SELECT @VP_K_FOLIO=(INVENTARIO.K_FOLIO)
				FROM	INVENTARIO
				WHERE	K_ITEM=@PP_K_ITEM
				AND		LOTE_PEARL=@PP_LOTE_PEARL
				AND		K_ORDEN_COMPRA_PEDIDO	=@PP_K_ORDEN_COMPRA_PEDIDO
				...
				UPDATE	FOLIO
				SET		K_LOCACION=@PP_K_LOCACION_DESTINO
				WHERE	K_FOLIO=@VP_K_FOLIO
				
				IF @@ROWCOUNT = 0
					BEGIN
						--RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
						SET @VP_MENSAJE='El Folio no fue actualizado.'
						RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
					END
		END
		...
		IF @VP_MENSAJE=''
		BEGIN
			CURSOR INSERTAR REGISTROS INDEPENDIENTES POR LOTE
		END
-- //////////////////////////////////////////////////////////////
GO
...
---- // STORED PROCEDURE ---> INSERT EN LA TABLA DE MOVIMIENTO
---- //////////////////////////////////////////////////////////////
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_IN_MOVIMIENTO_MQU_TO_MHI]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_IN_MOVIMIENTO_MQU_TO_MHI]
GO

CREATE PROCEDURE [dbo].[PG_IN_MOVIMIENTO_MQU_TO_MHI]
	@PP_K_SISTEMA_EXE				INT,
	@PP_K_USUARIO_ACCION			INT,
	-- ===========================
	@PP_K_ORDEN_COMPRA_PEDIDO		[VARCHAR](50),
...
---- //////////////////////////////////////////////////////////////
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_IN_MOVIMIENTO_MHI_TO_MQU]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_IN_MOVIMIENTO_MHI_TO_MQU]
GO

CREATE PROCEDURE [dbo].[PG_IN_MOVIMIENTO_MHI_TO_MQU]
	@PP_K_SISTEMA_EXE				INT,
	@PP_K_USUARIO_ACCION			INT,
	-- ===========================
	@PP_K_ORDEN_COMPRA_PEDIDO		[VARCHAR](50),
...
--=====================================================================================================================================
	...
	BEGIN
		...
		BEGIN			
				EXECUTE [dbo].[PG_IN_MOVIMIENTO_MHI_TO_MQU]	@PP_K_SISTEMA_EXE, @PP_K_USUARIO_ACCION
														,@PP_K_ORDEN_COMPRA_PEDIDO		,@PP_K_ENTREGA
														,@PP_K_ITEM						,@PP_LOTE_PEARL
														,@PP_K_LOCACION_ORIGEN			,@PP_K_LOCACION_DESTINO
		END
		...
		BEGIN			
				EXECUTE [dbo].[PG_IN_MOVIMIENTO_MQU_TO_MHI]	@PP_K_SISTEMA_EXE, @PP_K_USUARIO_ACCION
														,@PP_K_ORDEN_COMPRA_PEDIDO		,@PP_K_ENTREGA
														,@PP_K_ITEM						,@PP_LOTE_PEARL
														,@PP_K_LOCACION_ORIGEN			,@PP_K_LOCACION_DESTINO
		END
...
--		BEGIN
--			UPDATE	INVENTARIO_LOCACION
--			SET		CANTIDAD_DISPONIBLE=(CANTIDAD_DISPONIBLE +		@VP_QTY_MOVIMIENTO	)
--			WHERE	K_ITEM=				  @PP_K_ITEM	
--			AND		LOTE_PEARL=			  @PP_LOTE_PEARL
--		END
--	END
...
--	IF @VP_MENSAJE=''
--	BEGIN
--		EXECUTE [dbo].[PG_IN_MOVIMIENTO_X_INSPECCION_MATERIAL]		@PP_K_SISTEMA_EXE, @PP_K_USUARIO_ACCION
--														,@PP_K_ORDEN_COMPRA_PEDIDO		,@PP_K_ENTREGA
--														,@PP_K_ITEM						,@PP_LOTE_PEARL
--														,@PP_K_LOCACION_ORIGEN			,@PP_K_LOCACION_DESTINO
--														,@VP_QTY_MOVIMIENTO				,@VP_QTY_DESTINO
--	END	
----=====================================================================================================================================
----=====================================================================================================================================	
--		IF @VP_MENSAJE=''
...
--		END
--	----RAISERROR (@VP_ERROR_1, 16, 1 ) --MENSAJE - Severity -State.
--	--SET @VP_MENSAJE='ERROR PARA PRUEBAS.'
--	--RAISERROR (@VP_MENSAJE, 16, 1 ) --MENSAJE - Severity -State.
---- /////////////////////////////////////////////////////////////////////
--COMMIT TRANSACTION 
--END TRY
