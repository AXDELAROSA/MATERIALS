...
-- //////////////////////////////////////////////////////////////
-- ////////			CONTENIDO DEL SP
--	[PG_LI_INVENTARIO_HILO]
--	[PG_LI_ENTREGAR_CONTROLADO_EMPLEADOS]
--	[PG_SK_ENTREGAR_CONTROLADO_LOTE]
--	[PG_SK_ENTREGAR_CONTROLADO_EMPLEADO]
--	[PG_IN_ENTREGAR_CONTROLADO]
--	[PG_PR_ENTREGAR_CONTROLADO_UP_INVENTARIO]
	...
	BEGIN
		SELECT	DATENAME(DW,GETDATE()) + ', ' + CONVERT(varchar,GETDATE(),107)	AS F_DATE,
				...
				(CASE
					...
					WHEN	INVENTARIO.K_ITEM	= 1867	THEN	'GLOBAL BLACK'
...
-- // STORED PROCEDURE ---> LISTADO DE INVENTARIO ACTUAL EN ALMACEN DE PRODUCCIÓN
-- //////////////////////////////////////////////////////////////
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PG_LI_INVENTARIO_HILO_DISPONIBLE]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[PG_LI_INVENTARIO_HILO_DISPONIBLE]
GO
--		 EXECUTE [dbo].[PG_LI_INVENTARIO_HILO_DISPONIBLE] 1,139
CREATE PROCEDURE [dbo].[PG_LI_INVENTARIO_HILO_DISPONIBLE]
	@PP_K_SISTEMA_EXE				INT,
	@PP_K_USUARIO_ACCION			INT
	-- ===========================
AS
	...
	SELECT	D_ITEM,
			...
			LOTE_PEARL,
			LOTE_VENDOR,
			CONVERT(INT,(CANTIDAD_RECIBIDA/2)) AS QUANTITY
	FROM	DATA_02.DBO.INVENTARIO 
	INNER JOIN	FOLIO				(NOLOCK)	ON	FOLIO.K_FOLIO	= INVENTARIO.K_FOLIO
	INNER JOIN	COMPRAS.DBO.ITEM	(NOLOCK)	ON	ITEM.K_ITEM		= INVENTARIO.K_ITEM
	WHERE	K_LOCACION				= 125
	AND		K_CLASS_ITEM			= 2
	AND		TRADEMARK_ITEM			= 'HILO'
	ORDER BY PART_NUMBER_ITEM_PEARL	DESC	
	-- ////////////////////////////////////////////////////////////////////
GO
...
GO
--		 EXECUTE [dbo].[PG_LI_ENTREGAR_CONTROLADO_EMPLEADOS] 1,139, '2022-01-01','2022-04-30'
...
SELECT --	* ,
		...
		C_INVENTARIO						AS COMENTARIOS,
		MATERIAL_CONTROLADO.F_ALTA
FROM MATERIAL_CONTROLADO		(NOLOCK)	
INNER JOIN COMPRAS.DBO.ITEM		(NOLOCK)	ON	ITEM.K_ITEM	= MATERIAL_CONTROLADO.K_ITEM
INNER JOIN IMLOCFIL_SQL			(NOLOCK)	ON	IMLOCFIL_SQL.A4GLIdentity	= MATERIAL_CONTROLADO.K_LOCACION_ENTREGA
...
--		 EXECUTE [dbo].[PG_SK_ENTREGAR_CONTROLADO_LOTE] 0,139,'1776549'
...
--	DECLARE @VP_MENSAJE				VARCHAR(300) = ''	
	...
	INNER JOIN	INVENTARIO			(NOLOCK)	ON		INVENTARIO.LOTE_PEARL	= INVENTARIO_LOCACION.LOTE_PEARL
...
BEGIN
	...
	--===============================================================================================================================================================
	--===============================================================================================================================================================
	--IF @PP_K_ITEM IN (70,1507,1508,1867,1868,2213)
	--BEGIN
...
DECLARE		 @VP_CU_K_ORDEN_COMPRA_PEDIDO	VARCHAR(50)			,@VP_CU_K_ENTREGA			INT					,@VP_CU_LOTE_PEARL				INT
		...
		DECLARE CU_CANTIDAD_ENTREGAR CURSOR LOCAL STATIC READ_ONLY FORWARD_ONLY FOR
			...
			AND		K_LOCACION			= 125	-- PRD		---4 (MHI)					-- LOCACIÓN ORIGEN
			AND		K_ITEM				= @PP_K_ITEM		-- 70		--- 334		--	@PP_K_ITEM
			AND		LOTE_VENDOR			= @PP_LOTE_VENDOR	-- '645194'	---'Q20077'	--	@PP_LOTE_VENDOR
			---	AND		TIPO				= 'B'
